...
- services always point to pods(they do not point deployment or replica set etc.), service points to pods directly using labels.
- in k8s pods can communicate with each other by using IP addresses in whatever node the pod might be placed.
------------------------------------------------------------------------------------------------------------
Why do we need service?
    Kubernetes pods are ephemeral(lasting for a very short time) in nature. Deployment object(s) can create and destroy pods dynamically. 
Each pod does have it’s own IP address, hence in a deployment, the set of pods running change all the time, so do the IP address for the pods
